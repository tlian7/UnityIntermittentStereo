...
public class OVRCameraRig : MonoBehaviour
{
	...
	// TL
	public Text hmdOrientText;
	public Text debugText;
	public float binocularFOV = 60;
	static public float frequency = 1f / 180f;
	float leftUpper;
	float leftLower;
	float rightUpper;
	float rightLower;
...
#region Unity Messages
	...
	private void Start ()
	{
		...
		leftUpper = 270f + (180f - binocularFOV) / 2f;
		leftLower = 270f - (180f - binocularFOV) / 2f;
		rightUpper = 90f - (180f - binocularFOV) / 2f;
		rightLower = 90f + (180f - binocularFOV) / 2f;
	...
	private void UpdateAnchors ()
	{
		...
		debugText.text = "Good";
		float yRotation = centerEyeAnchor.localRotation.eulerAngles.y;
		ipdFraction = 1f;

		if (!KeyboardControl.sinusoidMode) {

			// If the rotation is outside the binocular FOV, reduce IPD
			// Too far right
			if ((yRotation > rightUpper && yRotation < rightLower)) {
				debugText.text = "Bad.";
				angleDelta = Mathf.Abs (yRotation - 90f);
				ipdFraction = angleDelta / ((180f - binocularFOV) / 2f);
			}
			// Too far left
			if ((yRotation < leftUpper && yRotation > leftLower)) {
				debugText.text = "Bad.";
				angleDelta = Mathf.Abs (yRotation - 270);
				ipdFraction = angleDelta / ((180f - binocularFOV) / 2f);
			}
		
		} else {
			ipdFraction = Mathf.Abs(Mathf.Cos (2f*Mathf.PI*frequency*yRotation));
			debugText.text = "Frequency = " + (1/frequency).ToString("F2") + " degrees per cycle";
		}
				
		//Change hmdEye positions
		hmdLeftEye.position = (hmdLeftEye.position - centerEyeAnchor.localPosition) * ipdFraction + centerEyeAnchor.localPosition;
		hmdRightEye.position = (hmdRightEye.position - centerEyeAnchor.localPosition) * ipdFraction + centerEyeAnchor.localPosition;
		currIPD = (hmdLeftEye.position - hmdRightEye.position).magnitude;

		// Display center rotation
		hmdOrientText.text = 
//			"x: " + centerEyeAnchor.localRotation.eulerAngles.x + "\n" + 
			"yRotation: " + yRotation.ToString("F3") + "\n" + 
//			"z: " + centerEyeAnchor.localRotation.eulerAngles.z + "\n" + 
				"ipd: " + (1000f*currIPD).ToString("F3") + "mm \n"+
				"ipdFraction = " + ipdFraction.ToString("F2");
//			+ "LeftEye: " + hmdLeftEye.position*100f + "\n" 
//				+ "CenterEye: " + centerEyeAnchor.localPosition *100f + "\n"
//			+ "RightEye: " + hmdRightEye.position*100f;
...
//		// Reverse the affect of positional tracking is toggle is true
